...
void compute_energy_matrix(const Image* img, Matrix* energy) {
    ...
    for (int i = 1; i < Matrix_height(energy) - 1; i++) {
        for (int j = 1; j < Matrix_width(energy) - 1; j++) {
            Pixel pNorth = Image_get_pixel(img, i - 1, j);
            Pixel pSouth = Image_get_pixel(img, i + 1, j);
            Pixel pWest = Image_get_pixel(img, i, j - 1);
            Pixel pEast = Image_get_pixel(img, i, j + 1);
            int sqd = squared_difference(pNorth, pSouth) 
                + squared_difference(pWest, pEast);
            *Matrix_at(energy, i, j) = sqd;
            if (sqd > max_sqd)max_sqd = sqd;
        }
    }
    Matrix_fill_border(energy, max_sqd);
...
void remove_vertical_seam(Image *img, const int seam[]) {
    ...
    for (int i = 0; i < Image_height(newImg); i++) {
        ...
        for (int j = 0; j < Image_width(newImg); j++) {
            if (col == seam[i])col++;
            Image_set_pixel(newImg, i, j, Image_get_pixel(img, i, col));
            col++;
        }
    }
...
void seam_carve_width(Image *img, int newWidth) {
    ...
    for (int i = 0; i < original_width - newWidth; i++) {
